...
単発の指示だけでなく、以下の3ステップを回すことで成果率が上がります。

1. **計画（Plan）** — タスクの目的・対象ファイル・完了条件を明確にしてから作業を始める
2. **実行（Execute）** — 計画に沿って変更を加える。スコープ外の修正は行わない
3. **検証（Verify）** — `python check_prompts.py` でバリデーションを実行し、意図した結果になっているか確認する

このサイクルを1タスクごとに回すことで、手戻りを減らし、CIの失敗を防ぎます。
